...
const tabs = [
    ...
    { name: 'Messages', href: 'details/messages' },
        ...
        <div className="border-b border-gray-200 dark:border-gray-700">
            <nav className="-mb-px flex space-x-8" aria-label="Tabs">
                {tabs.map((tab) => (
                    <Link
                        key={tab.name}
                        href={`/mydapps/${tab.href}/?appId=${appId}`}
                        className={`px-1 py-4 border-b-2 font-medium text-sm ${pathname === `/mydapps/${tab.href}/`
                        ...
                        {tabs.map((tab) => (
                            <MenuItems key={tab.name}>
                                {({ }) => (
                                    <Link
                                        href={`/mydapps/${tab.href}/?appId=${appId}`}
                                        className={`block px-4 py-2 text-sm ${pathname === `/mydapps/${tab.href}/`
